...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	BYTE commandB[2] = {reg + 2,val};

	int return_code = alt_avalon_spi_command(SPI_0_BASE,0,
										2,commandB,
										0,0,0);
	if(return_code<0){
		alt_printf("error occured \n");
	}


			//slave = 0
			//master = SPI_0_BASE from system.h
	}
